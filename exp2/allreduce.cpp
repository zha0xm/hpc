...
void Ring_Allreduce(void* sendbuf, void* recvbuf, int n, MPI_Comm comm, int comm_sz, int my_rank)
{
    ...
    memcpy(recvbuf, sendbuf, sizeof(float) * n);

    for (int i = 0; i < comm_sz - 1; i++) {
        int send_rank = (my_rank + 1) % comm_sz;
        int recv_rank = (my_rank - 1 + comm_sz) % comm_sz;

        float* send_addr = (float*)sendbuf + ((my_rank - i + comm_sz) % comm_sz) * chunk_size;
        float* recv_addr = (float*)recvbuf + ((my_rank - 1 - i + comm_sz) % comm_sz) * chunk_size;
    ...
    for (int i = 0; i < comm_sz - 1; i++) {
        ...
        float* send_addr = (float*)recvbuf + (my_rank - i + 1 + comm_sz) % comm_sz;
        float* recv_addr = (float*)recvbuf + (my_rank - i + comm_sz) % comm_sz;
